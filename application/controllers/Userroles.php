...
class UserRoles extends CI_Controller {
	...
	function index(){
		...
		$this->load->view('roles_staff_list', $data);
	}
	
	function UserInfo($id){
		$this->load->helper(array('form'));
		
		$personalData = $this->user->getPersonalData($id);
		
		$data['staff_list'] = $this->staff->getStaffList();
		$data['user_id'] = $id;
		
		$data['first_name'] = $personalData->first_name;
		$data['last_name'] = $personalData->last_name;
		
		$data['roles_list'] = $this->roles->getRoles();
		$data['roles_id'] = $personalData->role_id;
		
		
		$data['department_name'] = $this->department->getDepartmentById($personalData->department_id);
		$data['roles_name'] = $this->roles->getRolesById($personalData->role_id, $personalData->department_id);
		
		//var_dump($data['department_list']);
		//die(0);
		
// 		$data['emp_id'] = $personalData['emp_id'];
// 		$data['department_id'] = $personalData['department_id'];
// 		$data['role_id'] = $personalData['role_id'];
// 		$data['reports_to'] = $personalData['reports_to'];
		
		$this->load->view('user_roles_manage', $data);
	}
